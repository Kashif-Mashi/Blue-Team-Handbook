...
1. **Which database stores local user account information on a standalone Windows machine?**
   ...
   - C) Domain Naming Service (DNS)
   - D) Registry SYSTEM Hive

2. **What is the RID associated with the built-in local Administrator account?**
   - A) 500
   - B) 501
   - C) 512
   - D) 1000

3. **Which built-in Windows account context possesses complete control over local OS resources?**
   - A) `LOCAL SERVICE`
   - B) `NETWORK SERVICE`
   - C) `NT AUTHORITY\SYSTEM`
   - D) `DefaultAccount`

4. **What unique string identifier is assigned to every Windows identity?**
   - A) GUID
   - B) SID
   - C) MAC Address
   - D) SSL Hash

5. **Which command displays the current user's security privileges and group memberships?**
   - A) `net accounts`
   - B) `whoami /all`
   - C) `systeminfo`
   - D) `get-host`

6. **Which Windows Event ID indicates that a new user account was created?**
   - A) Event ID 4624
   - B) Event ID 4720
   - C) Event ID 4732
   - D) Event ID 7045

7. **What is the default integrity level assigned to standard user applications launched under UAC?**
   - A) Low Integrity
   - B) Medium Integrity
   - C) High Integrity
   - D) System Integrity

8. **Which Windows feature automatically randomizes local administrator passwords across enterprise machines?**
   - A) BitLocker
   - B) LAPS
   - C) AppLocker
   - D) Credential Guard

9. **Which Event ID logs when a user is added to a local security group?**
   - A) Event ID 4732
   - B) Event ID 4688
   - C) Event ID 1102
   - D) Event ID 4625

10. **Which service host process manages authentication tokens and credential storage in memory?**
    - A) `svchost.exe`
    - B) `lsass.exe`
    - C) `csrss.exe`
    - D) `smss.exe`
...
2. **A** (500)
3. **C** (`NT AUTHORITY\SYSTEM`)
4. **B** (SID)
5. **B** (`whoami /all`)
6. **B** (Event ID 4720)
7. **B** (Medium Integrity)
8. **B** (LAPS)
9. **A** (Event ID 4732)
10. **B** (`lsass.exe`)
